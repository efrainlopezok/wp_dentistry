...
class CreateLazyImg {
	...
	public function lazyloadBackgroundImages( $html, $buffer ) {
		...
		if ( ! preg_match_all( '#<(?<tag>div|section|article|a)\s+(?<before>[^>]+[\'"\s])?style\s*=\s*([\'"])(?<styles>.*?)\3(?<after>[^>]*)>#is', $buffer, $elements, PREG_SET_ORDER ) ) {
			return $html;
		}
	...
	private function addLazyClass( $element ) {
		if ( preg_match( '#class=["\']?(?<classes>[^"\'>]*)["\']?#is', $element, $class ) ) {
			$classes = str_replace( $class['classes'], $class['classes'] . ' pre-lazyload', $class[0] );
			$element = str_replace( $class[0], $classes, $element );

			return $element;
		}

		return preg_replace( '#<(img|div|section|article|a)\s*([^>]*)>#is', '<\1 class="pre-lazyload"\2>', $element );
	}
	...
	private function replaceImage( $image ) {
		...
		$image_lazyload = preg_replace( '/([\s"\'])srcset/i', '\1data-lazy-srcset', $image_lazyload );

		$image_lazyload = preg_replace( '/([\s"\'])sizes/i', '\1data-lazy-sizes', $image_lazyload );
